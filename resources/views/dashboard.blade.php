...
<x-app-layout>
    <div class="p-4 grid gap-6">
        ...
        @if($current_user["access_level"] !== $userLevel)
                            ...
                            @if($current_user["access_level"] === $adminLevel)
                                <td><select name="access_level" class="access_level" data-source="{{$user["id"]}}">
                                        <option value="{{$adminLevel}}"
                                                @if ($adminLevel === $user["access_level"]) selected @endif>{{$adminLevel}}</option>
                                        <option value="{{$teamLeadLevel}}"
                                                @if ($teamLeadLevel === $user["access_level"]) selected @endif>{{$teamLeadLevel}}</option>
                                        <option value="{{$userLevel}}"
                                                @if ($userLevel === $user["access_level"]) selected @endif>{{$userLevel}}</option>
                                    </select>
                                </td>
                            @endif
                        </tr>
